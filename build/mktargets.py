...
def find_sources():
    ...
    for dir in os.walk("."):
        for file in dir[2]:
            if len(INCLUDE) == 0 and file not in EXCLUDE or file in INCLUDE:
...
with open(OUTFILE, "w") as f:
    ...
    if mipsfiles:
        mmifiles = [file for file in mipsfiles if '_mmi' in file]
        f.write("%s_ASM_MIPS_MMI_SRCS=\\\n"%(PREFIX))
        for c in mmifiles:
            f.write("\t$(%s_SRCDIR)/%s\\\n"%(PREFIX, c))
        f.write("\n")
        f.write("%s_OBJSMIPS_MMI += $(%s_ASM_MIPS_MMI_SRCS:.c=.$(OBJ))\n\n"%(PREFIX, PREFIX))
        msafiles = [file for file in mipsfiles if '_msa' in file]
    ...
    if loongarchfiles:
        lsxfiles = [file for file in loongarchfiles if '_lsx' in file]
        f.write("%s_ASM_LOONGARCH_LSX_SRCS=\\\n"%(PREFIX))
        for c in lsxfiles:
            f.write("\t$(%s_SRCDIR)/%s\\\n"%(PREFIX, c))
        f.write("\n")
        f.write("%s_OBJSLOONGARCH_LSX += $(%s_ASM_LOONGARCH_LSX_SRCS:.c=.$(OBJ))\n\n"%(PREFIX, PREFIX))
        lasxfiles = [file for file in loongarchfiles if '_lasx' in file]
